    ...
    if (context.inline) {
        ...
        return (
            <ContentPrimitive
                ...
                {...popoverContentProps}
                className={cn('flex flex-col gap-4 p-4 w-[340px]', className)}
            >
                {children}
            </ContentPrimitive>
        );
...
function ColorPickerArea(props: ColorPickerAreaProps) {
    ...
    return (
        <AreaPrimitive
            ...
            {...areaProps}
            className={cn(
                'relative border rounded-sm w-full h-40 touch-none cursor-crosshair',
        ...
        >
            ...
            <div
                className="absolute shadow-sm border-2 border-white rounded-full size-3 -translate-x-1/2 -translate-y-1/2"
                style={{
                    left: `${hsv?.s ?? 0}%`,
                    top: `${100 - (hsv?.v ?? 0)}%`,
                }}
...
function ColorPickerHueSlider(props: ColorPickerHueSliderProps) {
    ...
    return (
        ...
        >
            <SliderPrimitive.Track className="relative bg-[linear-gradient(to_right,#ff0000_0%,#ffff00_16.66%,#00ff00_33.33%,#00ffff_50%,#0000ff_66.66%,#ff00ff_83.33%,#ff0000_100%)] rounded-full w-full h-3 overflow-hidden grow">
                <SliderPrimitive.Range className="absolute h-full" />
            </SliderPrimitive.Track>
            <SliderPrimitive.Thumb className="block bg-background disabled:opacity-50 shadow border border-primary/50 rounded-full focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring size-4 transition-colors disabled:pointer-events-none" />
        </SliderPrimitive.Root>
    );
}
...
function ColorPickerAlphaSlider(props: ColorPickerAlphaSliderProps) {
    ...
    return (
        ...
        >
            <SliderPrimitive.Track
                className="relative rounded-full w-full h-3 overflow-hidden grow"
                style={{
                    background:
...
function ColorPickerSwatch(props: ColorPickerSwatchProps) {
    ...
    return (
        <SwatchPrimitive
            ...
            {...swatchProps}
            className={cn(
                'box-border shadow-sm border rounded-sm size-8',
...
const inputGroupItemVariants = cva(
    'focus-visible:z-10 [&::-webkit-inner-spin-button]:m-0 [&::-webkit-outer-spin-button]:m-0 focus-visible:ring-1 h-8 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none [-moz-appearance:_textfield]',
    {
        variants: {
            position: {
